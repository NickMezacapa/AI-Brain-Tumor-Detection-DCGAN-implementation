...
PATH = os.path.abspath(
    "E:/UB CSE/Spring 2018/700/Project/BRATS2013/BRATS_Training/BRATS-2/Image_Data")  # Load the dataset
...
def pad_image(img, desired_shape=(256, 256)):
    ...
    pad_left = 0
    pad_right = 0
    # if desired height is greater than image height, pad top and bottom
    if desired_shape[0] > img.shape[0]:
        pad_top = int((desired_shape[0] - img.shape[0]) / 2)
        pad_bot = desired_shape[0] - img.shape[0] - pad_top
    # if desired width is greater than image width, pad left and right
    ...
    img = img[50:200, 50:200]  # crop image to standard size
...
def normalize(img):
    nimg = None
    nimg = cv.normalize(img.astype('float'), nimg, alpha=0.0,
                        beta=1.0, norm_type=cv.NORM_MINMAX)
...
def load_single_image(path):
    for dir, subdir, files in os.walk(path):
        for file in files:
            # if the file is a .mha file, load it
...
def load_dataset(path):
    ...
    for dir in os.listdir(path):
        # load images in HG directory
        if dir == 'HG':
            HG_path = os.path.join(path, 'HG')
            for dir2 in os.listdir(HG_path):  # load images in each subdirectory
                if dir2 != '.DS_Store':  # ignore .DS_Store file
                    # Load ground truth and flair images
                    HG_flair = load_single_image(os.path.join(
                        HG_path, dir2, 'VSD.Brain.XX.O.MR_Flair'))
                    HG_ot = load_single_image(os.path.join(
                        HG_path, dir2, 'VSD.Brain_3more.XX.XX.OT'))
        ...
        # load images in LG directory
        if dir == 'LG':
            brain_1 = brain_2 = brain_3 = False
            LG_path = os.path.join(path, 'LG')
            for dir3 in os.listdir(LG_path):
                if dir3 != '.DS_Store':
                    LG_flair = load_single_image(os.path.join(
                        LG_path, dir3, 'VSD.Brain.XX.O.MR_Flair'))
                    brain_1 = os.path.exists(os.path.join(
                        LG_path, dir3, 'VSD.Brain_1more.XX.XX.OT'))
                    brain_2 = os.path.exists(os.path.join(
                        LG_path, dir3, 'VSD.Brain_2more.XX.XX.OT'))
                    brain_3 = os.path.exists(os.path.join(
                        LG_path, dir3, 'VSD.Brain_3more.XX.XX.OT'))
                    if brain_1:
                        LG_ot = load_single_image(os.path.join(
                            LG_path, dir3, 'VSD.Brain_1more.XX.XX.OT'))
                    if brain_2:
                        LG_ot = load_single_image(os.path.join(
                            LG_path, dir3, 'VSD.Brain_2more.XX.XX.OT'))
...
#    print("gen,z:",z.shape)
    ...
    with tf.variable_scope('generator', reuse=False if is_train == True else True):
        ...
        logits = tf.layers.conv2d_transpose(
            lrelu4, out_channel_dim, 5, 2, padding='SAME')
...
#     example_z = np.random.uniform(-1, 1, size=[n_images, z_dim])
    example_z = np.reshape(
        flair_data[420, :, :], (1, IMAGE_WIDTH*IMAGE_HEIGHT))
    samples = sess.run(
    ...
    with tf.Session() as sess:
        ...
        for epoch_i in range(epoch_count):
            for batch_images, batch_z in get_batches(batch_size):
                ...
                steps += 1
                batch_z = np.reshape(
                    batch_z, (batch_size, IMAGE_WIDTH*IMAGE_HEIGHT))
